...
// Stands in for core's HotkeyPoller in the oracle library only, so the published
// build's Hotkeys::Start (oracle/src/hotkeys.cpp, verbatim) runs with no polling
// thread and the test sees what it registered. Compiled with the oracle's
// renaming, so everything here lives in dxhr_oracle_core and cannot collide with
// the real poller the current core links.
//
// The GetAsyncKeyState below is declared in the namespace chord_hotkeys.h's
// guards are defined in, so their unqualified calls find it before
// ::GetAsyncKeyState. That is how the test holds modifiers without a keyboard.
